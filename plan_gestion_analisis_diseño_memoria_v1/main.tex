...
A lo largo de las siguientes secciones vamos a profundizar un poco más sobre la gestión del proyecto.  \\

En la sección 2 trataremos la organización del proyecto, sus integrantes, la división en equipos, y los roles adoptados por cada uno de ellos. \\

En la sección 3, hablaremos sobre el plan de gestión del proyecto: plan inicial de trabajo, control de configuraciones, aseguramiento de la calidad del producto y división de las tareas mediante un calendario establecido para cada equipo.  \\

Por otro lado, la sección 4 abordará cuestiones de análisis y diseño del sistema: descripción de requisitos funcionales y no funcionales, modelado de diagrama de paquetes, de componentes, de despliegue y una descripción de las tecnologías utilizadas.  \\

Finalmente, en la sección 5 describiremos el proceso de trabajo seguido por cada uno de los equipos: inicio del proyecto y desarrollo del mismo, el control del rendimiento de los integrantes de cada equipo y, finalmente, sobre el cierre del trabajo.  \\
...
A continuación, se describe cómo se han llevado a cabo distintas tareas a realizar en el proyecto: plan inicial de trabajo, división del trabajo a realizar o control de creación de software, entre otros.

\subsection{Inicio del proyecto}

Los integrantes del equipo desconocíamos varias de las tecnologías y lenguajes de programación utilizados en este proyecto. Para solventar este problema, hemos tenido que realizar la siguiente formación común:

\begin{itemize}

\item Debido a que toda la documentación necesaria para el proyecto va a ser realizada en \textbf{Latex}, todos los miembros debíamos adquirir conocimientos básicos para la utilización y explotación de esta herramienta. Para lograr este objetivo, hemos realizado dos tutoriales. El primero de ellos, <<Learn LaTeX in 30 minutes>>\footnote{\href{https://www.overleaf.com/learn/latex/Learn_LaTeX_in_30_minutes}{\color{blue}{Learn LaTeX in 30 minutes}}}, se puede encontrar en la página web de \textit{Overleaf}. A su vez, hemos seguido los manuales de ayuda proporcionados por la página oficial de \LaTeX \footnote{\href{https://latex-tutorial.com/tutorials/}{\color{blue}{Tutorial oficial de \LaTeX}}}.

\item Continuando lo anterior, los integrantes del equipo asistimos a la práctica: \textit{Puesta en marcha de GitHub y fundamentos de Git} impartida por los profesores de la asignatura \textit{Proyecto Software}, la cual consistió en familiarizarse con la herramienta \textbf{Git} y su uso en conjunto con \textbf{GitHub}. A su vez, por medio del tutorial: \textit{Git, GitHub y Publicación Web}\footnote{\href{https://github.com/flowsta/github}{\color{blue}{Git, GitHub y Publicación Web}}} hemos adquirido los conocimientos básicos de la misma.

\end{itemize}

Los grupos encargados de cada bloque del sistema han realizado formación específica. En primer lugar, los encargados del desarrollo del \textbf{backend}:

\begin{itemize}

\item Harán uso de \textbf{Go} como lenguaje de programación, al tener experiencia previa gracias a las prácticas de la asignatura de \textit{Sistemas Distribuidos}. De cara a fijar conocimientos y a aprender nuevas mecánicas, han realizado el tutorial de la página web oficial de \textbf{Go}.

\item A su vez, han hecho una formación relacionada con las distintas \textbf{API web}, concretamente el tutorial \textit{Introducción a las API web} de MozillaDeveloper\footnote{\href{https://developer.mozilla.org/es/docs/Learn/JavaScript/Client-side_web_APIs/Introduction}{\color{blue}{Introducción a las API web}}}.
...
\item  Este grupo ha realizado el tutorial oficial de \textbf{ReactJs}: \textit{Tutorial: Introducción a React}\footnote{\href{https://es.reactjs.org/tutorial/tutorial.html}{\color{blue}{Tutorial: Introducción a React}}}, que tiene una duración estimada de 8 horas. También, esta pareja ha completado el tutorial de API web anteriormente mencionado.
...
\item Ha realizado el tutorial \textit{Angular y TypeScript}\footnote{\href{https://angular.io/tutorial}{\color{blue}{Tutorial: Angular y TypeScript}}} proporcionado por \textit{DesarrolloWeb}, el cual tiene un tiempo estimado de realización próximo a las 7 horas.
...
\end{landscape}
 ...
 Para el diseño de las pantallas se realizó un boceto simple de estas, el cual se tomará como referencia en mayor medida por ambos \textit{frontend}.\footnote{\href{https://github.com/UNIZAR-30226-2022-01/proyecto_software_documentacion/tree/main/imagenes}{\color{blue}{\textit{Diseño inicial}}}}.
\\
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.88\linewidth]{diagramas/DP.pdf}
...
Concretamente, en la Figura \ref{ref:paquetes} se puede apreciar que para el frontend de React y de Angular se ha seguido una estructura de paquetes distinguiendo entre las páginas de la aplicación y los componentes que se utilizan en estas, cada uno con su archivo \textit{.jsx} y \textit{.css} para React y  \textit{.ts}, \textit{.spec.ts}, \textit{.html} y \textit{.css} para Angular. Por otro lado, para el backend, el módulo principal se centra en el paquete \textit{handlers}, el cual depende de la representación del modelo de datos (\textit{vo, dao}) y de la lógica interna del juego (\textit{logica\_juego}), entre otros, así como la existencia del módulo de test con las pruebas en referencia al paquete del servidor. \\

En siguiente lugar, se ha modelado el \textbf{diagrama de componentes}, apreciable en la Figura \ref{ref:componentes}, en el cual se puede apreciar los dos componentes cliente, uno para cada tecnología. Estos implementan la lógica de presentación, es decir, todo lo referente a la estructura de las páginas (\textit{html}) y su estilo (\textit{css}), y contienen parte de lógica interna de la aplicación, correspondiente a todo el código que gestiona formularios de entrada del usuario (\textit{iniciarSesion, crearPartida...}) o el estado local de una partida en curso (\textit{mapa, cartas...}), y se comunican con el servidor mediante la misma interfaz (\textit{API}), a través de las llamadas implementadas en cada uno de los paquetes que componen las páginas del diagrama de paquetes. En lo referido al almacenamiento local, este comprende datos en cuanto a cookies de usuario o variables comunes a todos los componentes, tales como el nombre de usuario. \\

En siguiente lugar, se encuentra el servidor \textit{web} para la \textit{API}, el cual ofrece una interfaz común a los clientes (\textit{handlers}) e implementa la lógica de negocio (\textit{logica\_juego}) y el acceso a los datos del sistema (\textit{vo, dao, modelo}). Por último, se encuentra el gestor de base de datos, el cual aporta la persistencia necesaria al sistema. Además, como \textbf{protocolo de comunicación} entre los nodos de la red, se utilizará \textit{HTTPS}.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.88\linewidth]{diagramas/DC.pdf}
...
Por otro lado, en la Figura \ref{ref:despliegue} se ilustra el \textbf{diagrama de despliegue}, donde se puede apreciar la distribución de los distintos nodos que componen el sistema. En consecuencia, se ha representado el servidor web de la \textit{API}, contenido en un entorno de ejecución correspondiente a un contenedor de \textit{Docker}, que ejecuta el código del servidor, y otro entorno con el contenedor para el SGBD \textit{PostgreSQL}. Por otro lado, se encuentran los servidores de React y Angular, cada uno con su entorno de ejecución con un contenedor de \textit{Docker}, que proporcionan el cliente web correspondiente. Por último, se encuentra la máquina del usuario, la cual contiene el entorno de ejecución (navegador web), junto con los artefactos necesarios para ejecutar la aplicación cliente correspondiente. \\

\begin{figure}[!h]
    \centering
    \includegraphics[width=1\linewidth]{diagramas/DD.pdf}
...
En la Figura \ref{ref:relacional} aparece el modelo relacional utilizado para la base de datos de la aplicación. Este constará de 6 tablas:
\begin{itemize}
...
A grandes rasgos, los objetivos que establecimos en el inicio del proyecto han sido cumplidos: hemos realizado los tutoriales anteriormente mencionados y, en muchos casos, han resultado insuficientes y hemos tenido que indagar y buscar más información sobre aspectos más complejos como pueden ser el uso y explotación de intervalos en \textit{React} y \textit{Angular} o sobre el funcionamiento y deshabilitación de CORS, entre otros. \\ 

En relación al equipo del frontend de \textbf{React}, hemos seguido de forma correcta el plan establecido. De cara a entrar en contacto con el framework, hemos realizado un proyecto de prueba para poner en práctica las competencias adquiridas y desplegar una primera aplicación muy sencilla. También hemos investigado y probado diversos paquetes y librerías adicionales de \textit{React}, y hemos aprendido a realizar las conexiones con la API y a gestionar cookies. Cabe mencionar que los objetivos que planteamos en la planificación inicial los hemos cumplido y esto nos permitió poder empezar a trabajar y desarrollar el frontend con una buena base de conocimientos. \\

% Apartado backend
Con lo que respecta al equipo de \textbf{backend}, la formación inicial necesaria no fue tan pesada como en el caso del \textit{frontend}, ya que ambos componentes del equipo tenían experiencia con las tecnologías utilizadas. Aún así ambos consultaron tutoriales y documentación de \textit{React} y \textit{Angular} para poder ayudar en el desarrollo del \textit{frontend}, además de los tutoriales sobre diseño de \textit{API} y uso de \LaTeX. \\
...
El trabajo desarrollado por el equipo de \textit{React} ha contado con una muy buena comunicación y coordinación por parte de sus integrantes debido, en gran medida, a haber trabajado juntos en múltiples ocasiones. Esta situación ha propiciado que el entorno de trabajo haya sido cordial y dinámico. La coordinación y el modelo de trabajo han seguido las siguientes indicaciones:
...
\begin{itemize}
    ...
    \item Por otro lado, en el resto de ejecuciones se ha optado por realizar dos reuniones semanales de cara a poner en común aspectos realizados, fijar distintos objetivos a lograr o especificar diseños de pantallas. Conforme se acercaba la fecha final de entrega estas reuniones han ido aumentado progresivamente.
    \item Por último, semanalmente se han repartido tareas individuales no excesivamente extensas de forma equitativa.
\end{itemize}

Cabe destacar que, después de la entrega intermedia, este equipo pasó a estar formado por tres integrantes, ya que Guillermo se unió al desarrollo de este frontend. Pese a ello, la comunicación y aspectos mencionados anteriormente han sido mantenidos y el equipo ha logrado tener un gran nivel de comunicación y coordinación. \\

Como se ha mencionado, el \textbf{control de trabajo} lo han producido en las múltiples reuniones realizadas. Por otro lado, se han utilizado los \textit{issues de GitHub}, de cara a mostrar las tareas realizadas y las pendientes de realizar. Respecto al seguimiento de los procedimientos y herramientas establecidas en un inicio, cabe destacar:

\begin{itemize}
    \item La tecnología elegida inicialmente ha sido adecuada para el desarrollo del sistema, y ha facilitado en gran medida la implementación del mismo, debido a la gran cantidad de información y formaciones que se puede encontrar por la \textit{web} relacionada con \textit{React}.
    ...
    \item Cabe destacar que se han añadido o modificado múltiples librerías y paquetes con respecto a los establecidos al inicio del proyecto. Esto se ha debido a que se han ido descubriendo progresivamente ciertos paquetes que, en un principio, no se contaba con utilizar. Los más destacables son:
        \begin{itemize}
            \item \textit{React-router-dom}\footnote{\href{https://v5.reactrouter.com/web/guides/quick-start}{\color{blue}{\textit{Página oficial react-router-dom.}}}}: navegar entre las distintas pantallas. 
            \item \textit{SweetAlert2}\footnote{\href{https://sweetalert2.github.io}{\color{blue}{\textit{Página oficial SweetAlert2.}}}}: alertas auxiliares personalizables.
            \item \textit{Query String}\footnote{\href{https://www.npmjs.com/package/query-string}{\color{blue}{\textit{Página oficial query-string.}}}}: parser de URL.
            \item \textit{Boostrap}\footnote{\href{https://getbootstrap.com}{\color{blue}{\textit{Página oficial bootstrap.}}}}: framework de desarrollo web que incluye diseños de plantillas en HTML y CSS.
        \end{itemize}
\end{itemize}

%% Calendario inicial vs Calendario final

A lo largo del proyecto, el \textbf{calendario inicial} ha sufrido múltiples modificaciones y en muchos aspectos no hemos conseguido seguirlo completamente y ha sido necesario realizar modificaciones al mismo. \\

Para empezar, de cara a la entrega intermedia no pudimos completar diversas tareas como las pantallas de \textit{Perfil de usuario}, \textit{Búsqueda de partida} y \textit{Buzón de notificaciones}. Los motivos principales de estos cambios se deben a que, en primer lugar, se subestimó el coste de varias tareas, como la conexión con la API, ya que el primer mes de proyecto el avance fue más lento de lo esperado debido a que aun estábamos terminando de adaptarnos a las tecnologías utilizadas. Por otro lado, se puso un mayor énfasis en la pantalla del \textit{Mapa} frente a las anteriores debido a que se trata de la más compleja de implementar y desarrollar, ya que tiene una importancia relevante sobre todas las demás por toda la lógica de jugabilidad que comprende. \\

Con respecto a la entrega final fue necesario volver a adaptar el calendario para incluir las tareas retrasadas y para reducir el tiempo de desarrollo del mapa. A su vez, cabe destacar que a estas alturas del proyecto dominábamos bastante más la tecnología y esto nos permitió reducir en gran medida el tiempo de desarrollo de múltiples funcionalidades de la aplicación. La estimación inicial consistía en acabar la funcionalidad completa del juego el 27 de Mayo, objetivo que fue alcanzado el día 15 del mismo mes. Este gran adelanto con respecto al plan inicial nos permitió dedicarle más tiempo a mejorar el diseño de las pantallas y a dedicar más tiempo a realizar más test exploratorios. \\

%% Problemas que han surgido

Por último, durante el desarrollo de este frontend hemos tenido múltiples \textbf{problemas}. Los más destacables serían:

\begin{itemize}
    \item Debido a que hemos implementado un juego que requiere un desarrollo secuencial, ha sido necesario incluir intervalos que regularan el transcurso del mismo y que permitieran trabajar con el modelo de sincronía del lenguaje. El problema principal reside en que conforme avanzamos implementando más funcionalidad del juego, se añadían más intervalos, lo que al final ocasionaba un gran problema ya que unos se solapaban con otros. Este aspecto es sin duda que más problemas nos ha ocasionado. 
    
    \item Ocurrió un problema al desplegar el frontend web debido a que existían incompatibilidades entre \textit{macOS} y los archivos iniciales de\textit{Docker}. Tras revisar y estudiar el error en profundidad el equipo consiguió solucionar el problema.
\end{itemize}
...
\begin{itemize}
    \item Al realizar pruebas exploratorias de los \textit{frontend} con el despliegue automático de contenedores de \textit{Docker}, se han experimentado problemas no observados al realizar los test automáticos debido a que los servidores de \textit{frontend} y \textit{backend} se encontraban en dominios diferentes.
...
Para comprobar el correcto rendimiento de los miembros del equipo, así como la evolución del mismo a lo largo del desarrollo, se han recogido y evaluado diferentes métricas de \textit{Github} al final del mismo. Esto ha sido posible gracias a que se ha hecho uso de \textit{issues} e hitos para realizar el control de tareas, y a que \textit{Github} recoge métricas de \textit{commits} realizados y sus cambios.

\subsubsection{Control de incidencias} 
...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.32]{metricas/code_churn_backend.png}
...
En primer lugar, cabe destacar que este proyecto ha sido nuestra primera toma de contacto seria en el desarrollo web desarrollando una aplicación de unas dimensiones y características superiores a lo que estábamos acostumbrados. \\

Este detalle ha ocasionado que no estimáramos bien en un inicio la carga total de trabajo o, más bien el tamaño, costes y esfuerzo que íbamos a tener que emplear para llevar a cabo el proyecto. Esta falta de experiencia se ha denotado en el calendario inicial ya que, aunque finalmente se han logrado todos los objetivos, al final prácticamente se ha tenido que ir rehaciendo sobre la marcha mientras avanzábamos. \\

Otra lección importante aprendida sería la evolución que hemos tenido programando esta aplicación. En el primer mes de proyecto únicamente fueron creadas tres pantallas a un ritmo bastante lento debido a la falta de práctica con este framework. Por otro lado, en las dos primeras semanas de Mayo, programamos 8 pantallas incluyendo toda la lógica del juego. Esta programación a distintas velocidades no fue tenida en cuenta al inicio, estimamos un desarrollo lineal durante todo el proyecto, pero como se ha visto nuestro rendimiento y eficiencia aumentaron gradualmente. \\

A su vez, hemos aprendido la importancia de la modularización de código ya que desde un primer momento seguimos el mismo patrón dividiendo pantallas y componentes. Hemos visto la verdadera importancia de tener un código limpio organizado en módulos para proyectos de un tamaño considerable, ya que de otra manera hubiera resultado una tarea mucho más costosa. \\ 

En cuanto a la propia tecnología, por regla general, nos hemos sentido agusto trabajando con el framework de \textit{React}, aunque ciertos aspectos como la rápida actualización de librerías, funciones... han complicado en sobremanera ciertas tareas debido a que existe una extensa documentación, pero la mayor parte de ella está obsoleta. \\

También, desde un primer momento englobamos la lógica del juego como única tarea pero como vimos posteriormente se debería haber dividido en 4 distintas: refuerzo, ataque, fortificar y cartas. \\

Por último, destacar que, pese a que nos hemos encontrado con diversos errores debido a la falta de experiencia, nos sentimos satisfechos ya que consideramos que hemos desarrollado cierta versatilidad para hacer frente a los errores y para conseguir completar todos los objetivos planteados al inicio del proyecto. \\
...
\begin{itemize}
    ...
    \item \textbf{Fran}, el coordinador del grupo, para la primera entrega, desarrolló junto con el resto del grupo de \textit{frontend} el diseño del mapa de juego. Además, la implementación de diversas pantallas como \textit{Registro Usuario, Inicio Sesión y Crear Partida}, entre otras. También, realizó dinámica adicional del mapa como ciertos botones, un mapa informativo o pantallas de avisos. 
    
    De cara a la entrega final, realizó la implementación de la lógica del juego, así como otras pantallas como \textit{Buscar Partida}. También, realizó el diseño y estilo final de las pantallas, las pruebas finales sobre el sistema y se encargó de la redacción de apartados de la memoria y de actas. Empleó un total de 140 horas. 

    \item \textbf{Emilio} dedicó un total de 141 horas a las tareas asignadas, entre las que destacan, en la primera entrega, la puesta en marcha inicial del cliente, conexiones con la API y su gestión de errores, mantenimiento de cookies, implementación y diseño de pantallas y elementos gráficos, y redacción, maquetación y diseño de apartados de la memoria. 
    
    Para entrega final, se encargó principalmente de la implementación de la lógica del juego y las distintas fases que lo comprenden, así como de la interacción con las cartas en una partida, el diseño y estilo final de las pantallas y las pruebas finales sobre el sistema. También realizó las tareas de documentación asignadas para la presentación del proyecto y memoria final.
...
\begin{itemize}
    \item Se ha comprobado que el modelo de sincronía del lenguaje no es el más adecuado para el juego secuencial desarrollado, de forma que es interesante explorar otras opciones más orientadas al desarrollo y modelado de videojuegos, como Unity. Asimismo, nos hubiera gustado profundizar con más detalle en el modelo de concurrencia y gestión de hilos, ya que, dados los problemas que surgieron en relación a estos aspectos, el hecho de conocer el modelo que existe por debajo hubiera facilitado el funcionamiento de muchas de las fases de la partida.
    
    \item Hasta la etapa de aportar estilo a las pantallas, el equipo no se dividió prácticamente ninguna tarea, evitando paralelizar el máximo trabajo posible y la eficiencia en el avance de la funcionalidad. Sin embargo, ya que se contaban con muy pocos conocimientos sobre la tecnología, no fue una mala decisión desarrollar la mayoría de la aplicación con \textit{pair programming}. 
    
    \item No se cumplió siempre el requisito realizar \textit{commits} con un tamaño considerable y con sentido, ya que en muchas ocasiones se subían demasiadas funcionalidades y cambios de una sola vez.
    
    \item También hubiera sido útil modularizar algún aspecto adicional de lógica que compartían las pantallas, como las llamadas a la API o muchas de las partes del funcionamiento del juego, distinguiendo, por ejemplo, entre las distintas fases del juego y agrupando funciones en distintos paquetes, ya que esa parte del código resultó desestructurada y mejorable. 
    
    \item Dada la falta de experiencia, se infravaloró el tiempo estimado para las tareas, sobre todo al comienzo del proyecto, en relación a la falta de conocimientos de la tecnología. En consecuencia, muchas de las tareas simples iniciales, como la conexión con la API, llevaron demasiado tiempo, y otras más complejas, como el desarrollo del juego, fueron, en proporción, menos costosas.
...
\begin{itemize}
    ...
    \item  Nos hemos dado cuenta que el desarrollo web y la gestión de proyectos no es una tarea nada sencilla y requiere de un equipo comprometido, trabajador y de una buena sinergia y comunicación entre todos los integrantes.
    
    % ¿¿Comentar aquí algo de un desarrollo más paralelo para facilitar las interfaces de la api o cómo recuperar el estado al compartir...??
...
\subsection{Acta de reunión 1}
\includepdf[]{actas/Acta1.pdf}

\thispagestyle{empty}
\includepdf[]{actas/Acta2.pdf}

\thispagestyle{empty}
\includepdf[]{actas/Acta3.pdf}

\thispagestyle{empty}
\includepdf[]{actas/Acta4.pdf}

\thispagestyle{empty}
\includepdf[]{actas/Acta5.pdf}
